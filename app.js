...
(function () {
    ...
    window.handleSignup = async function (event) {
        ...
        if (!validGmail(email)) return showToast('error', 'Email must be a valid @gmail.com address.');
    ...
    window.handleLogout = async function () {
        try {
            ...
            state.user = { ...state.user, redeemedBalance: 0 };
    ...
    function purchaseFailureModal(message, retry) {
        openPurchaseModal(`
            <div class="purchase-modal-success purchase-modal-failure">
                ...
                <p class="purchase-modal-copy">You have not been charged for this attempt.</p>
                <p class="purchase-modal-error">${escape(message)}</p>
                <div class="purchase-modal-actions one-action"><button class="btn btn-primary" id="purchaseRetry" type="button">Try Again</button></div>
            </div>
        `);
        byId('purchaseFailureClose').onclick = closePurchaseModal;
        byId('purchaseRetry').onclick = retry;
    }

    function paymentRedirectModal() {
        openPurchaseModal(`
            <div class="purchase-process" aria-label="Opening secure payment">
                <span class="purchase-spinner" aria-hidden="true"></span>
    ...
    function paymentNoticeModal({ kicker, title, copy, primaryText = 'Close', onPrimary = closePurchaseModal }) {
        openPurchaseModal(`
            <div class="purchase-modal-success purchase-modal-failure">
                <button class="purchase-modal-close" id="paymentNoticeClose" type="button" aria-label="Close message">×</button>
                <span class="purchase-failure-mark" aria-hidden="true">!</span>
                <p class="purchase-modal-kicker">${escape(kicker)}</p>
                <h2>${escape(title)}</h2>
                <p class="purchase-modal-copy">${escape(copy)}</p>
                <div class="purchase-modal-actions one-action"><button class="btn btn-primary" id="paymentNoticeAction" type="button">${escape(primaryText)}</button></div>
            </div>
        `);
        byId('paymentNoticeClose').onclick = closePurchaseModal;
        byId('paymentNoticeAction').onclick = onPrimary;
    }
    ...
    // Tapping BUY goes straight to payment: the server opens a checkout for exactly the
    // card price and the card is issued once the payment is confirmed.
    ...
    async function completePurchase(cardId, idempotencyKey, retried = false) {
        ...
        const request = (async () => {
            try {
                const data = await api('/api/purchases', { method: 'POST', body: JSON.stringify({ cardId, idempotencyKey, returnOrigin: window.location.origin }) });
                if (data.purchase) {
                    // This key already produced a card (paid earlier, browser never came back).
                    clearPurchaseRequestKey(cardId);
                    if (!retried) { purchaseRequests.delete(idempotencyKey); return completePurchase(cardId, purchaseRequestKey(cardId), true); }
                    applyServerState(data.state, { refreshUI: false, reason: 'card-payment' });
                    refreshMountedUI('card-payment');
                ...
                if (!checkoutUrl) throw new Error('Payment could not be started. Please try again.');
    ...
    // The customer lands here after paying. The server has already re-verified the charge
    // with the payment hub before redirecting, so this only reads back the result.
    async function handleCardPaymentReturn() {
        const params = new URLSearchParams(location.search);
        ...
        processingPurchaseModal();
        try {
            const data = await api(`/api/card-payments/${encodeURIComponent(transactionId)}`);
            applyServerState(data.state, { refreshUI: false, reason: 'card-payment-verified' });
            safeNavigate('shop', 'card-payment-verified');
            refreshMountedUI('card-payment-verified');
            const status = data.payment.status;
            if (status === 'SUCCESS' && data.purchase) {
                clearPurchaseRequestKey(data.payment.cardId);
                purchaseSuccessModal(data.purchase, state.cards.find(card => card.id === data.purchase.cardId));
            } else if (status === 'PENDING' || status === 'PAYMENT_INITIALIZED') {
                paymentNoticeModal({ kicker: 'Payment pending', title: 'We are still confirming your payment.', copy: 'This usually takes a few seconds. Your card will appear under Redeem as soon as it is confirmed.', primaryText: 'Check again', onPrimary: () => { closePurchaseModal(); history.replaceState({}, '', `/?card_payment=1&transactionId=${encodeURIComponent(transactionId)}`); handleCardPaymentReturn(); } });
            } else if (status === 'PAID_UNFULFILLED' || status === 'PAID_DUPLICATE') {
                paymentNoticeModal({ kicker: 'Payment received', title: 'We could not issue this card automatically.', copy: `Your payment went through but the card could not be issued. Please contact support with reference ${data.payment.reference} and we will refund or resolve it.` });
            } else {
                paymentNoticeModal({ kicker: 'Payment not completed', title: 'Your payment was not completed.', copy: 'You have not been charged. You can try again whenever you are ready.' });
            }
            return true;
        } catch (e) {
            closePurchaseModal();
            safeNavigate('shop', 'card-payment-verification-failed');
            showToast('error', e.message);
            return false;
        }
    }

    function handlePaymentOutcomeFromUrl() {
        const outcome = new URLSearchParams(location.search).get('payment');
        history.replaceState({}, '', location.pathname || '/');
        safeNavigate('shop', 'payment-outcome');
        if (outcome === 'review') {
            paymentNoticeModal({ kicker: 'Payment received', title: 'We could not issue this card automatically.', copy: 'Your payment went through but the card could not be issued. Please contact support and we will refund or resolve it.' });
        } else if (outcome === 'expired') {
            paymentNoticeModal({ kicker: 'Payment expired', title: 'This payment session expired.', copy: 'You have not been charged. Tap Buy again to start a new payment.' });
        } else {
            paymentNoticeModal({ kicker: 'Payment not completed', title: 'Your payment was not completed.', copy: 'You have not been charged. You can try again whenever you are ready.' });
        }
    }
    ...
    const SHOP_PRICE_RANGES = [
        { key: 'starter', label: '$4–$5', min: 4, max: 5 },
    ...
    function purchasePriceKey(card) {
        ...
        // Buying two cards anywhere inside a price tier (e.g. one $4 card and
        // one $5 card in the Starter tier) exhausts the whole tier for the
        // day — the daily purchase cap is tracked per tier, not per exact price.
        const tier = SHOP_PRICE_RANGES.find(range => price >= range.min && price <= range.max);
    ...
    window.renderShopCards = function () {
        ...
        if (priceFilter !== 'all') {
            selectedRange = SHOP_PRICE_RANGES.find(item => item.key === priceFilter);
            if (selectedRange) filtered = filtered.filter(card => Number(card.price) >= selectedRange.min && Number(card.price) <= selectedRange.max);
        }
        ...
        container.innerHTML = `
            <div class="card" style="background:var(--bg-card);">
                ...
                <div style="display:flex;gap:16px;flex-wrap:wrap;margin:12px 0 16px;">
                    ...
                    <div><span class="text-sm text-muted">You pay</span><div class="text-xl font-bold">${ghs(price)}</div></div>
    ...
    function renderWalletBoard() {
        const redeemed = byId('wbsRedeemedValue');
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        try {
            await refresh();
            const params = new URLSearchParams(location.search);
            if (params.has('card_payment')) await handleCardPaymentReturn();
            else if (params.has('payment')) handlePaymentOutcomeFromUrl();
            else if (params.has('kyc_bypass_reference') || params.has('kyc_bypass_test')) await completeKycBypassFromUrl();
            else safeNavigate('shop', 'initial-refresh');
        } catch (e) {
            state.isLoggedIn = false;
            safeNavigate('login', 'initial-refresh-failed');
            const params = new URLSearchParams(location.search);
            if (params.has('card_payment') || params.has('payment') || params.has('kyc_bypass_reference') || params.has('kyc_bypass_test')) showToast('error', e.message);
        } finally {
